...
float V_alfa_inter[2] = {1000}, V_alfa_out[2] = {1000}, V_alfa_int[2] = {1000};
float V_beta_inter[2] = {1000}, V_beta_out[2] = {1000}, V_beta_int[2] = {1000};
float V_alfa_P = 1000, V_alfa_N = 1000, V_beta_P = 1000, V_beta_N = 1000;
float FLL_int[2] = {1000}, FLL_Freq[2] = {1000}, Freq_in = 0, Freq_Rede = 376.99111841;
float Teta = 0;
int first_time = 0;
...
float Amostras_Teta[167] = {0}; // Variável para armazenamento da tensao da fase A da rede
unsigned int counter = 0; // Contador para a variável Amostras_TensaoA.
...
void main(void)
{
    ...
    EPwm2Regs.TBCTL.bit.CTRMODE = 0;    // Contagem up
    EPwm2Regs.TBPRD = 7500;         // Frequencia do PWM = 10 kHz
...
void Gpio_select(void)
{
    ...
    GpioCtrlRegs.GPADIR.bit.GPIO10 = 1; // GPIO12 = Teste da PLL
...
*/
    ...
    Vcc[AMOSTRAS-1] = KI_PCI*Kad*(float)AD_output7;
...
// Função para estimar o periodo e amplitude máxima de um sinal
//struct DadosEstimados Data_Estimation(float *Voltage)
//{
//    int ii = 0;                                     // Variável temporária para contagem
//    int Cont_Zero = 0;                              // Variável auxiliar para a contagem do numero de passagem por zero
//    float Value1 = 0, Value2 = 0, Value3 = 0;       // Variáveis temporárias para armazenamento dos valores de tensão do vetor de entrada
//    float Zero[3] = {0,0,0};                        // Variável para armazenamento dos instantes de passagem por zero
//    float Derivada = 0;                             // Variável para armazenamento da derivada dos sinais de tensão
//    struct DadosEstimados Dados;                    // Variável para armazenamento do valor de pico e frequência estimada
//    int Fator = 0;
//
...
//    Dados.Zero          = 0;
//
//    //Leitura do array, dos valores mais recentes (ultimas posições) para os mais antigos (posições iniciais)
//    for (ii=0;ii<(AMOSTRAS-1);ii++)
//    {
//        // Leitura dos valores de tensão existentes no vetor de entrada
//        Value1 = *(Voltage+AMOSTRAS-1-ii);
//        Value2 = *(Voltage+AMOSTRAS-2-ii);
//
//        // Detecção da passagem por zero: verificado a partir da mudança de sinal das tensões de entrada.
//        // Uma vez detectada a passagem por zero, o instante estimado é calculado a partir da derivada do sinal.
//        // Destaca-se que o intervalo entre duas amostras é igual ao periodo de amostragem
//        if (Value1*Value2 < 0)
//        {
//            // Cálculo da derivada do sinal
//            //Derivada = (Value2 - Value1)/PERI_AMOSTRAGEM;
//            Derivada = (Value1 - Value2)/PERI_AMOSTRAGEM;
//
//            // Armazenamento do instante de passagem por zero
//            //Zero[Cont_Zero] = ii*PERI_AMOSTRAGEM - Value1/Derivada;
//            Zero[Cont_Zero] = ii*PERI_AMOSTRAGEM + Value1/Derivada;
//
//            // Armazenamento da última passagem por zero
//            Dados.Zero = Zero[0];
//
//            //Incrementa o contador de passagem por zero
//            Cont_Zero++;
//
//            // Calculo da frequencia do sinal, a partir de duas passagens por zero (meio periodo)
//            if (Cont_Zero >= 3)
//                Dados.Frequencia = Dados.Frequencia + 1/(Zero[2]-Zero[0]);
////          if (Cont_Zero >= 2)
////              Dados.Frequencia = ( Dados.Frequencia + 1/(2*(Zero[Cont_Zero-1]-Zero[Cont_Zero-2])) )/(Cont_Zero-1);
//
//            // Se três passagens por zero foram detectadas, a execução da função é finalizada
//            if (Cont_Zero >= 3)
//                return Dados;
//
//            if (Derivada > 0)
...
//        if (Value2 == 0)
//        {
//            // Condição para detectar passagens por zero quando um dos valores lidos é zero
//
//            // Armazenamento do instante de passagem por zero
//            Zero[Cont_Zero] = (ii+1)*PERI_AMOSTRAGEM;
//
//            // Armazenamento da última passagem por zero
//            Dados.Zero = Zero[0];
//
//            //Incrementa contador de passagem por zero
//            Cont_Zero++;
//
//            // Calculo da frequencia do sinal, a partir de duas passagens por zero (meio periodo)
//            if (Cont_Zero >= 3)
...
//
//        /*
//         * Determinação da amplitude do sinal: comparação de três valores consecutivos
//         * para verificar a existência de um ponto de inflexão. Caso exista um ponto de inflexão,
//         * armazena-se o valor da tensão
//         */
//        if ( ((Value3*(Value2 - Value1)) < 0) && (Dados.Amplitude == 0) )
//        {
//            // Verificação do maior valor existente na amostra das tensões
//            //if ( (Value1*Value1) > (Dados.Amplitude*Dados.Amplitude))
//            //{
//                if (Fator == 0)
//                    Dados.Amplitude = Value1;
//                else
//                {
//                    if (Value1 <= 0)
//                        Dados.Amplitude = -Value1;
...
//                        Dados.Amplitude = Value1;
//
//                    Dados.Amplitude = Fator*Dados.Amplitude;
//
//                    if ( ((Fator == 1) && (Dados.Amplitude < 0)) || ((Fator == -1) && (Dados.Amplitude > 0)) )
//                        Dados.Amplitude = -Dados.Amplitude;
//                }
//            //}
//
//            //Armazenamento da diferença entre os dois valores, para uso na próxima iteração
